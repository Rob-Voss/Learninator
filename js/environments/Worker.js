...
(function (global) {
	...
	Life.World = function (parameters) {
		...
		this.setInputs = function () {
			for (var id in _this.agents) {
				...
				for (var id2 in _this.agents) {
					...
					if (agent.x < agent2.x - dist || agent.x > agent2.x + dist || agent.y > agent2.y + dist || agent.y < agent2.y - dist)
						continue;
					...
					if (d < dist) {
						...
						if (Math.abs(forwardAngle) > Math.PI)
							diff = 2 * Math.PI - Math.abs(forwardAngle);
						diff = Math.abs(diff);
					...
					for (var id2 in _this.agents) {
						...
						if (d < parameters.agent.size) {
							...
							if (Math.abs(diff) < Math.PI / 8) {
								...
								if (agent.boost)
									speedMultiplier = parameters.agent.sprintMultiplier;
		...
		this.sexuallyReproduce = function () {
			...
			for (var i = 0; i < _this.agents.length; i++) {
				if (_this.agents[i].age > _this.agents[i1].age && Math.random() < 0.1)
					i1 = i;
				if (_this.agents[i].age > _this.agents[i2].age && Math.random() < 0.1 && i != i1)
					i2 = i;
			}
		...
		this.asexuallyReproduce = function (agent, mutationRate1, mutationRate2) {
			if (Math.random() < 0.04)
				mutationRate1 *= Math.random() * 10;
			if (Math.random() < 0.04)
				mutationRate2 *= Math.random() * 10;
	...
	Life.Box = function (parameters) {
		...
		for (var i = 0; i < parameters.brain.connections; i++) {
			...
			if (Math.random() < 0.2)
				_this.id[i] = Math.round(Math.random() * parameters.brain.inputSize);
