...
/**
 * Represents a simulation, consisting of a toroidal grid of cells, each of which may
 * contain food, and a set of Agents.
...
Life.World = function (parameters) {
    ...
    this.processOutputs = function () {
        for (var id in this.agents) {
            ...
            if (agent.pos.x < 0) agent.pos.x = this.parameters.width + agent.pos.x;
            if (agent.pos.x >= this.parameters.width) agent.pos.x = agent.pos.x - this.parameters.width;
            if (agent.pos.y < 0) agent.pos.y = this.parameters.height + agent.pos.y;
            if (agent.pos.y >= this.parameters.height) agent.pos.y = agent.pos.y - this.parameters.height;
    ...
    this.sexuallyReproduce = function () {
        ...
        for (var i = 0; i < this.agents.length; i++) {
            if (this.agents[i].age > this.agents[i1].age && Math.random() < 0.1) i1 = i;
            if (this.agents[i].age > this.agents[i2].age && Math.random() < 0.1 && i != i1) i2 = i;
        }
...
Life.Agent = function (parameters) {
    ...
    this.mutate = function () {
        ...
        if (Math.random() < 0.1) this.mutationRate[0] = Life.Utils.randomNormal(this.mutationRate[0], _parameters.agent.mutationRate[0]);
        if (Math.random() < 0.1) this.mutationRate[1] = Life.Utils.randomNormal(this.mutationRate[1], _parameters.agent.mutationRate[1]);
        if (this.mutationRate[0] < 0.001) this.mutationRate[0] = 0.001;
        if (this.mutationRate[1] < 0.02) this.mutationRate[1] = 0.02;
        ...
        //Mutate clocks
        if (Math.random() < this.mutationRate[0] * 5) this.clock1 = Life.Utils.randomNormal(this.clock1, this.mutationRate[1]);
        if (this.clock1 < 2) this.clock1 = 2;
        if (Math.random() < this.mutationRate[0] * 5) this.clock2 = Life.Utils.randomNormal(this.clock2, this.mutationRate[1]);
        if (this.clock2 < 2) this.clock2 = 2;

        //Mutate senses
        if (Math.random() < this.mutationRate[0] * 5) this.smellModifier = Life.Utils.randomNormal(this.smellModifier, this.mutationRate[1]);
        if (Math.random() < this.mutationRate[0] * 5) this.soundModifier = Life.Utils.randomNormal(this.soundModifier, this.mutationRate[1]);
        if (Math.random() < this.mutationRate[0] * 5) this.hearingModifier = Life.Utils.randomNormal(this.hearingModifier, this.mutationRate[1]);
        if (Math.random() < this.mutationRate[0] * 5) this.eyeSenseModifier = Life.Utils.randomNormal(this.eyeSenseModifier, this.mutationRate[1]);
        if (Math.random() < this.mutationRate[0] * 5) this.bloodModifier = Life.Utils.randomNormal(this.bloodModifier, this.mutationRate[1]);
        ...
        for (var i in this.eyes) {
            var eye = this.eyes[i];
            if (Math.random() < this.mutationRate[0] * 5) eye.fov = Life.Utils.randomNormal(eye.fov, this.mutationRate[1]);
            if (Math.random() < this.mutationRate[0] * 5) eye.direction = Life.Utils.randomNormal(eye.direction, this.mutationRate[1]);
    ...
    this.mutate = function (mutationRate1, mutationRate2) {
        for (var i = 0; i < _parameters.brain.size; i++) {
            ...
            if (Math.random() < mutationRate1) {
                ...
                if (this.boxes[i].damper < 0.001) this.boxes[i].damper = 0.001;
                if (this.boxes[i].damper > 1) this.boxes[i].damper = 1;
            }

            if (Math.random() < mutationRate1) {
                this.boxes[i].globalWeight += Life.Utils.randomNormal(0, mutationRate2);
                if (this.boxes[i].globalWeight < 0) this.boxes[i].globalWeight = 0;
            }
...
Life.Brain.Box = function (parameters) {
    ...
    for (var i = 0; i < _parameters.brain.connections; i++) {
        ...
        if (Math.random() < 0.5) this.weight[i] = 0;

        this.id[i] = Math.round(Math.random() * (_parameters.brain.size - 1));
        if (Math.random() < 0.2) this.id[i] = Math.round(Math.random() * (_parameters.brain.inputSize - 1));
    ...
    this.setParameters = function (parameters) {
        if (JSON.encode(parameters.brain) != JSON.encode(_parameters.brain)) {
            ...
            for (var i = 0; i < parameters.brain.connections; i++) {
                ...
                if (Math.random() < 0.2) this.id[i] = Math.round(Math.random() * (parameters.brain.inputSize - 1));
