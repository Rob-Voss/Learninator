(function (global) {
    ...
    Cube.length = function (h) {
        return ((Math.abs(h.x) + Math.abs(h.y) + Math.abs(h.z)) / 2);
    };
        ...
        cartesianToHex: function (p) {
            ...
            if (this.orientation) {
                ...
                var q1 = 0.66666666666666663 * p.x,
                    r1 = -0.333333333333333315 * p.x + Math.sqrt(3) / 3 * p.y;
                return new FractionalCube(q1, -q1 - r1, r1);
            }
        },
        bounds: function () {
            var _g = this,
                centers = Lambda.array(this.hexes.map(function (hex) {
                    return _g.hexToCenter(hex);
                })),
                b1 = Grid.boundsOfPoints(this.polygonVertices()),
                b2 = Grid.boundsOfPoints(centers);
    ...
    global.Hex = Hex;
    global.Cube = Cube;
    global.FractionalCube = FractionalCube;
    global.Grid = Grid;
    global.HxOverrides = HxOverrides;
    global.Lambda = Lambda;
    global.ScreenCoordinate = ScreenCoordinate;
