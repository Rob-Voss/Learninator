(function (global) {
        ...
        var CollisionDetector = function (opts) {
            ...
            this.check = function (target) {
                ...
                //     } else if (entity.radius !== undefined && target.radius === undefined) {
                //         // Is it an entity versus a wall?
                //         // collisionObj = self.circleLineCollide(target, entity);
                //         collisionObj = self.lineCircleCollide(target, entity);
                //     }
                //
                //     if (collisionObj) {
             ...
             * @param {Wall} The wall or line
             * @param {Entity} circle Target
             * @returns {{vecI: Vec, [vecIs: Array,] distance: number}}
             */
            this.circleLineCollide = function (line, circle) {
                let collisionObj,
                    nearest,
                    lp1 = line.v1.sub(circle.pos),
                    lp2 = line.v2.sub(circle.pos),
                    lp2MinusLp1 = lp2.sub(lp1),
                    a = lp2MinusLp1.lengthSq(),
                    b = 2 * lp2MinusLp1.dot(lp1),
                    c = lp1.lengthSq() - (circle.radius * circle.radius),
                    delta = b * b - (4 * a * c);
                ...
                if (delta < 0) {
                    ...
                    collisionObj = {
                        vecI: vecI,
                        distance: circle.pos.distanceTo(vecI)
                    };
                } else if (delta > 0) {
            ...
            this.lineCircleCollide = function (line, circle, nearest) {
                ...
                if (gy2 > -circle.radius && gy2 < vy1) {
                    ...
                    gx1 = cos * gx2 - sin * gy2;
                    gy1 = cos * gy2 + sin * gx2;
                    collisionObj.vecI = new Vec();
                    collisionObj.vecI.vx = cos * vx1 - sin * vy1;
                    collisionObj.vecI.vy = cos * vy1 + sin * vx1;
                    collisionObj.vecI.x = line.v1.x + gx1;
                    collisionObj.vecI.y = line.v1.y + gy1;
                }
                //check collision
        ...
        var QuadCD = function () {
            ...
            this.drawRegions = function (aNode) {
                ...
                rect.clear();
                rect.lineStyle(1, 0xFF0000, 0.5);
                rect.moveTo(aNode.corners[0].x, aNode.corners[0].y);
                rect.lineTo(aNode.corners[1].x, aNode.corners[1].y);
                rect.lineTo(aNode.corners[2].x, aNode.corners[2].y);
                rect.lineTo(aNode.corners[3].x, aNode.corners[3].y);
                rect.lineTo(aNode.corners[0].x, aNode.corners[0].y);
                rect.endFill();
            ...
            this.drawRegions = function () {
                ...
                if (this.cheats.grid) {
                    ...
                    this.grid.cells.forEach((cell) => {
                        ...
                        rect.clear();
                        rect.lineStyle(1, 0xFF0000, 0.5);
                        rect.moveTo(cell.corners[0].x, cell.corners[0].y);
                        rect.lineTo(cell.corners[1].x, cell.corners[1].y);
                        rect.lineTo(cell.corners[2].x, cell.corners[2].y);
                        rect.lineTo(cell.corners[3].x, cell.corners[3].y);
                        rect.lineTo(cell.corners[0].x, cell.corners[0].y);
                        rect.endFill();
                        ...
                        let txtOpts = {font: "10px Arial", fill: "#000000", align: "center"},
                            ...
                            centerXY = this.grid.getCenterXY(cell);
                        popText.position.set(centerXY.x, centerXY.y);
                        rect.addChild(popText);
                        this.collisionOverlay.addChild(rect);
                    });
