 ...
 *
 * tree.retrieve(selector, callback) calls the
 * callback for all objects that are in
...
var QuadTree = function (args) {
  ...
  /**
   * Node creator.
   * You should never create a node manually since the
   * algorithm takes care of that for you.
   * @param {number} x
   * @param {number} y
   * @param {number} width
   * @param {number} height
   ...
   * @return {{x: *, y: *, width: *, height: *, depth: *, retrieve: Function, insert: Function, findInsertNode:
     *     Function, findOverlappingNodes: Function, divide: Function, clear: Function, getNodes: Function}}
   */
  var node = function (x, y, width, height, depth, maxChildren, maxDepth) {
    ...
    return {
      ...
      /**
       * Iterates all items that match the selector and invokes
       * the supplied callback on them.
       ...
       *
       * If the node already has sub nodes, the item gets
       * pushed down one level.
       * If the item does not fit into the sub nodes, it
       * gets saved in the "children"-array.
       *
      ...
      insert: function (item) {
        if (nodes.length) {
          ...
          items.push(item);
          // Divide the node if maxChildren is exceeded
          // and maxDepth is not reached
       ...
       * ---------------------------------------------------------
       * This returns this nodes sub nodes.
       * this is useful if we want to do stuff
       * with the nodes, i.e. accessing the bounds
       * of the nodes to draw them
