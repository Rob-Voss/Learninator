...
function draw() {
	...
	for (var i = 0, n = w.agents.length; i < n; i++) {
		var agent = w.agents[i],
			...
			rewColor = (reward > 255) ? 255 : ((reward < 0) ? 0 : reward),
			avgRew = brain.average_reward_window.get_average().toFixed(2),
			loc = ((i * 100) + n) + 10;

		ctx.fillStyle = "rgb(" + rewColor + ", 150, 150)";
		ctx.strokeStyle = "rgb(0,0,0)";

//		ctx.font = "10px Courier";
//		ctx.fillText(i + " Avg: " + avgRew, loc, 8);

		// Draw agents body
		ctx.beginPath();
		ctx.arc(agent.oldPos.x, agent.oldPos.y, agent.rad, 0, Math.PI * 2, true);
		ctx.fill();
		ctx.fillText(i + ":" + avgRew, agent.oldPos.x + agent.rad * 2, agent.oldPos.y + agent.rad * 2);
		ctx.stroke();
