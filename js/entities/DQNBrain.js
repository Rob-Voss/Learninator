(function (global) {
    ...
    self.onmessage = function (e) {
        ...
        switch (data.cmd) {
        case 'init':
            importScripts('../lib/external/rl.js', '../lib/Utility.js', '../lib/Window.js');
            var oEnv = JSON.parse(data.input.env),
                oOpts = JSON.parse(data.input.opts);
            _DQNBrain = new DQNBrain(oEnv, oOpts);
        ...
        case 'learn':
            ...
            self.postMessage({cmd: 'backward', msg: 'complete', input: 1});
            break;
        case 'stop':
